...
function alterarLivro() {
    if(livroParaAlterar){
        ...
        if(novoTitulo && novoAutor && novoAno){
            ...
            document.getElementById9("form-alterar").classList.add("hidden");
        } else {
            alert("Por favor, preencha todos os campos.")
        }
    }
...
function emprestimo() {
    const livroemprest = document.getElementById("nomeemprest").value;
    const usuário = document.getElementById("usuario").value;
    let resemp = document.getElementById("resemp"); 
    let buscarlivroemprestar = biblioteca.find((livro) => 
        livro.titulo == livroemprest
    );
    if (buscarlivroemprestar === livroemprest){
        resemp.innerHTML = `Livro emprestado ${livroemprest} Usuário: ${usuário}`;
        
    } else {
        resemp.innerHTML = `livro não encontrado.`
        resemp.innerHTML = `${buscarlivroemprestar}`
    }
}